...
signed main(){
    ...
    for(;;) {
        ...
        string s;
        cin>>s;
        vector<int> csum0(n+1,0),csum1(n+1,0);
        rep(i,n){
            csum0[i+1]=csum0[i]+(s[i]=='o');
            csum1[i+1]=csum1[i]+(s[i]=='_');
        }
        int res = 0;
        rep(i,n){
            //i日目をスタートにして、何日まで伸ばせるか？
            int ng = n+1-i, ok = 0;
            while(abs(ok-ng)!=1){
                int mid = (ok+ng)/2;
                // i日目からmid日伸ばす
                int val = csum0[mid+i] - csum0[i] + csum1[mid+i] - csum1[i];
                if(val == mid && csum1[mid+i] - csum1[i] <= k){
                    ok=mid;
                }else{
                    ng=mid;
                }
