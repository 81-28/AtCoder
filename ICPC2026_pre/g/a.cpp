...
v<map<int,vi>> g;
map<pii,vi> mp;
vi dfs(int pos,int cnt) {
    if (!cnt) return {0};
    if (mp.find({pos,cnt})!=mp.end()) return mp[{pos,cnt}];

    vi res;
    for (auto[nxt,cs]:g[pos]) {
        vi d=dfs(nxt,cnt-1);
        for (int c:cs)for(int val:d) res.pb(val^c);
    }
    return mp[{pos,cnt}]=res;
}
...
signed main(){
    ...
    for(;;) {
        ...
        rep(i,n) {
            vi d=dfs(i,l);
            for (int val:d) ans.pb(val);
        }
        int len=ans.size();
        sort(rall(ans));
        print(k>len?-1:ans[k-1]);
        // print(ans);
    }
