...
signed main(){
    ...
    for(;;) {
        ...
        vector<vector<int>> memoC(n,vector<int>(n,-1));

        auto cntC = [&](int l, int r) -> ll {
            if(memoC[l][r]!=-1)return memoC[l][r];
            deque<char> deq;
            ll res = 0;
            for(int i = l; i <= r; i++){
                deq.push_back(s[i]);
                ...
                while(1){
                    if(deq.size()<4)break;
                    if(deq.back() == 'C' && deq[deq.size()-2]=='P' && deq[deq.size()-3]=='C' && deq[deq.size()-4]=='I'){
                        rep(_,4) deq.pop_back();
                        deq.push_back('C');
                        res++;
                    }else{
                        break;
