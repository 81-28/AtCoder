...
//
// 対処（簡潔）:
//  - assign 系（上書き）では sentinel を使う（例: F=ll, id()=INF）。
...
constexpr ll INF = std::numeric_limits<ll>::max()/4; // 約2e18
...
struct S_add_sum { ll value; int size; };
using F_add_sum = ll;
...
//         より確実に衝突を避けたい場合は F を {bool have; ll v;} のような型にして no-op をフラグで表現する方が安全です。
using S_assign_min = ll;
using F_assign_min = ll;
...
using S_assign_max = ll;
using F_assign_max = ll;
...
struct S_assign_sum { ll value; int size; };
using F_assign_sum = ll;
