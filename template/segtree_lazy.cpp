...
public:
    ...
    // 要素にアクセスする関数
    int at(const int& pos) {return mx(pos,pos+1);}
};
...
// セグメントツリー（区間最小値）, 遅延評価（Lazy Propagation）対応
// 区間一括加算
class SegmentTree_min_add {
public:
    vector<int> vec,lazy;
    int size=1;
    SegmentTree_min_add(const int& n) {
        while(size<n)size*=2;
        vec=vector<int>(size*2,0);
...
public:
    ...
    // 要素にアクセスする関数
    int at(const int& pos) {return pr(pos,pos+1);}
};
