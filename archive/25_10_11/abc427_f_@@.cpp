...
int n,m;
// dp[i][j]:iで終わり、余りがjであるものの個数
v<map<int,int>> dp;
vi a;
// iで終わり、余りjであるものの個数
// cntだけ遡る
int dfs(const int& i,const int& j,const int& cnt) {
    if (dp[i].find(j)!=dp[i].end()) return dp[i][j];
    if (cnt<0) return 0;
    int res=a[i]==j;
    for (int k=0; k<i-1; ++k) {
        res+=dfs(k,(j-a[i]+m)%m,cnt-1);
    }
    return dp[i][j]=res;
}

// インデックスi,余りj,回数cntだけ加算する
void dfs2(const int& i,const int& j,const int& cnt,const int& add) {
    if (cnt<0) return;
    int r=(j+a[i])%m;
    dp[i][r]+=add;
    for (int k=i+2; k<n; ++k) {
        dfs2(k,r,cnt-1,dp[i][r]);
    }
}
...
signed main(){
    ...
    cin >> a;
    dp=v<map<int,int>>(n);
    rep(i,n) dfs2(i,0,n/4+1,1);
    int ans=1;
    rep(i,n) ans+=dfs(i,0,n/4+1);
    print(ans);
