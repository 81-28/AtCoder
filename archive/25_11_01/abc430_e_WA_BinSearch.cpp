...
// {sにtが含まれているか,その最初のidx}
pair<bool,int> check(const string& st,const string& t) {
    regex pat(t);
    auto it_begin = sregex_iterator(all(st),pat);
    auto it_end = sregex_iterator();
    if (it_begin!=it_end) return {1,(*it_begin).position()};
    else return {0,-1};
}
...
signed main(){
    ...
    while (t--) {
        ...
        if (check(b,a.substr(0,m)).f) {
            ...
            while (l<r) {
                m=(l+r)/2;
                if (check(b,a.substr(0,m)).f) l=m+1;
                else r=m;
            }
            if (b.substr(0,n-m)==a.substr(m,n-m)) print(m%n);
            else print(-1);
        } else {
            if (check(b,a.substr(m,n-m)).f) {
                // 後半が一致
                // 文字列[m,n)
                // 範囲[l,r)を探索
                int l=0,r=m;
                while (l<r) {
                    m=(l+r)/2;
                    if (check(b,a.substr(m,n-m)).f) r=m;
                    else l=m+1;
                }
                if (b.substr(n-m,m)==a.substr(0,m)) print(m);
                else print(-1);
            } else {
                print(-1);
            }
