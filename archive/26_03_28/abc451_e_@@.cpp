...
public:
    ...
    bool unite(const int& u, const int& v) {
        ...
        if (siz[rootU] < siz[rootV]) {
            par[rootU] = rootV;
            siz[rootV] += siz[rootU];
            rep(i,siz[rootU]) node[rootV].pb(node[rootU][i]);
        } else {
            par[rootV] = rootU;
            siz[rootU] += siz[rootV];
            rep(i,siz[rootV]) node[rootU].pb(node[rootV][i]);
        }
        return 0;
    }
...
signed main(){
    ...
    UnionFind uf(n);
    // 距離が短いのから確定させていく
    // つなげる2頂点の連結成分全てと辻褄が合うか確かめる
    while (!q.empty()) {
        auto[c,i,j]=q.top();
        q.pop();
        // 同一連結成分にある時、それで成立するかは連結時に確認済みである
        if (!uf.same(i,j)) {
            // 同一連結成分に無い時、それをつなげる方法は高々1通りしか存在しない
            int ri=uf.root(i);
            int rj=uf.root(j);
        }
    }
