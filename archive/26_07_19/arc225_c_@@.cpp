// https://atcoder.jp/contests/arc225/tasks/arc225_c
...
public:
    ...
    // 返り値はsameと同じ
    bool unite(const int& u, const int& v, const  int& c) {
        int rootU = root(u);
        int rootV = root(v);
...
signed main(){
    ...
    while (t--) {
        ...
        // 最小全域木完成状態から、置き換え可能なら置き換える
        // 置き換え可能：頂点a,bをたどる間にある重み0の辺の代わりに、重み1の辺a-bを使う
        // 現状、その連結成分にあるものは何でも消して良いことになっているのでWA
        if (sm<=k && k<=sm+rem) {
