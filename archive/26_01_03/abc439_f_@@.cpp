...
signed main(){
    ...
    vi p(n);
    cin >> p;
    // dp[i][j][k]:上り下りiで、位置kで、x-y=j-1のパターン数
    v<S> init(n+1,0);
    vector dp(2,vector(3,segtree<S,op,e>(init)));
    // dp[0][0].set(p[0],1);
    // dp[1][2].set(p[0],1);
    for (int i=0; i<n; ++i) {
        dp[0][0].set(p[i],dp[0][0].prod(p[i]+1,n+1));
        dp[0][1].set(p[i],dp[0][1].prod(p[i]+1,n+1) + dp[1][0].prod(p[i]+1,n+1));
        dp[0][2].set(p[i],1 + dp[0][2].prod(p[i]+1,n+1) + dp[1][1].prod(p[i]+1,n+1));
        dp[1][0].set(p[i],1 + dp[1][0].prod(1,p[i]) + dp[0][1].prod(1,p[i]));
        dp[1][1].set(p[i],dp[1][1].prod(1,p[i]) + dp[0][2].prod(1,p[i]));
        dp[1][2].set(p[i],dp[1][2].prod(1,p[i]));
    }
    print((dp[0][2].all_prod()+dp[1][2].all_prod()).val());
