...
map<pii,int> c;
vi d;
// pq[i]:頂点0からiまでに登場する辺の{重み,{0に近い頂点,頂点}}
v<priority_queue<pair<int,pii>>> pq;
void dfs(const int& m) {
    for (auto [nxt,b]:g[m]) {
        if (d[nxt]==-1) {
            d[nxt]=d[m]+1;
            pq[nxt]=pq[m];
            pq[nxt].push({c[{m,nxt}],{m,nxt}});
            dfs(nxt);
        }
    }
}
...
signed main(){
    ...
    pq=v<priority_queue<pair<int,pii>>>(n);
    d=vi(n,-1);
    d[0]=0;
    dfs(0);
    int ans=0;
    rep(i,n-1)for (int j=i+1; j<n; ++j) {
        priority_queue<pair<int,pii>> a=pq[i],b=pq[j];
        if (a.empty()) {
            ans+=c[b.top().s];
            continue;
        }
        ...
        while (a.top().s==b.top().s) {
            a.pop();
            b.pop();
        }
        ans+=max(c[a.top().s],c[b.top().s]);
    }
    print(ans);
