...
- **桁ごとの累積和を用いた巨大数の計算・出力**: [`abc233_e_large_number.cpp`](archive/26_05_28/abc233_e_large_number.cpp)
- **水位レベリング・累積値を2倍しながら必要水量を計算**: [`arc226_b_leveling.cpp`](archive/26_08_09/arc226_b_leveling.cpp)
- **多数決文字列の位置ペア条件を前計算し、問い合わせをO(m^2)で判定**: [`arc227_d_majority.cpp`](archive/26_08_16/arc227_d_majority.cpp)
...
- **直線上にk点以上ある直線の個数を数える・重複防止**: [`abc248_e_colinear.cpp`](archive/26_04_29/abc248_e_colinear.cpp)
- **多角形の重心・辺の外積による面積一次モーメント**: [`abc472_f_SegTree.cpp`](archive/26_08_22/abc472_f_SegTree.cpp)
...
- **ペアの最大XOR・未使用の最小iとそれ以外の未使用jと組ませて遷移する全探索**: [`abc236_d_all.cpp`](archive/26_05_24/abc236_d_all.cpp)
...
- **連続する1の塊の長さで勝敗判定・全0または全て長さ2の塊なら負け**: [`arc225_b_Nim.cpp`](archive/26_07_19/arc225_b_Nim.cpp)
- **最小値を引いた余剰と剰余で勝敗判定**: [`arc229_d_Nim.cpp`](archive/26_09_05/arc229_d_Nim.cpp)
...
- **区間DP（下限/上限付き配列に対する数え上げ）・累積和で高速化・modint**: [`abc222_d_mint_DP.cpp`](archive/26_06_15/abc222_d_mint_DP.cpp)
...
- **オイラーツアー・木に対するEuler Tour + セグメント木・辺の重み更新・パス和(区間和)・LCA・RMQ**: [`abc294_g_DFS_Euler_Tour_SegTree.cpp`](archive/25_12_16/abc294_g_DFS_Euler_Tour_SegTree.cpp)
...
- **部分木ごとに上位k個の値を保持・マージ**: [`abc239_e_DFS.cpp`](archive/26_05_19/abc239_e_DFS.cpp)
**行・列を両側に置き、各駒を中央に置く最大フロー（各駒を、同じ行・同じ列を共有しないように置ける最大個数を数える）**:[`abc205_f_DFS_MaximumFlow.cpp`](archive/26_07_02/abc205_f_DFS_MaximumFlow.cpp)
...
- **状態管理BFS・mapでベクトル状態をキーに距離管理(9パズル系)**: [`abc224_d_BFS_map.cpp`](archive/26_06_12/abc224_d_BFS_map.cpp)
- **木の辺更新を根基準の差分に落とし込み、BFSで伝播して各頂点の最終値を計算**: [`abc187_e_BFS_sum.cpp`](archive/26_08_17/abc187_e_BFS_sum.cpp)
...
- **連続方向制約グリッド・位置+向き(4状態)でDijkstraを行い経路復元**: [`abc453_d_dir_Dijkstra.cpp`](archive/26_04_11/abc453_d_dir_Dijkstra.cpp)
- **最短経路復元・親辺番号を記録**: [`abc252_e_Dijkstra_Parent.cpp`](archive/26_04_23/abc252_e_Dijkstra_Parent.cpp)
- **ダイクストラ・ワープ・超頂点**: [`abc463_e_Dijkstra.cpp`](archive/26_06_20/abc463_e_Dijkstra.cpp)

#### **ワーシャルフロイド・Floyd-Warshallアルゴリズム**
**キーワード**: `ワーシャルフロイド`, `Floyd-Warshall`, `全点対最短経路`, `3次元DP`, `全頂点間最短路`
- **頂点が昇順に使用可能になる・全点対最短経路の合計・(二次元改善版)**: [`abc208_d_Floyd-Warshall.cpp`](archive/26_07_01/abc208_d_Floyd-Warshall.cpp), [`abc208_d_2D_Floyd-Warshall.cpp`](archive/26_07_01/abc208_d_2D_Floyd-Warshall.cpp)
...
- **円の接触/重なりで連結判定**: [`abc259_d_UnionFind_circle_dist.cpp`](archive/26_04_10/abc259_d_UnionFind_circle_dist.cpp)
- **グリッド上の選択肢全探索・Union-Findで全て同一連結成分判定・外側からのBFSで穴判定**: [`abc219_e_bitAll_UnionFind_BFS.cpp`](archive/26_06_18/abc219_e_bitAll_UnionFind_BFS.cpp)
- **gcd完全グラフの最大全域木・値を大きい順に倍数走査して連結成分を統合**: [`abc469_f_UnionFind.cpp`](archive/26_08_01/abc469_f_UnionFind.cpp)
- **全域木を二色に塗り分けて二部グラフ作成・奇数長サイクル検出・パス復元**: [`abc472_e_bipartite_graph_UnionFind_BFS.cpp`](archive/26_08_23/abc472_e_bipartite_graph_UnionFind_BFS.cpp)
...
- **遅延セグ木・区間加算・区間和**: [`abc462_d_SegTree_Lazy.cpp`](archive/26_06_13/abc462_d_SegTree_Lazy.cpp)
...
- **多角形の重心・外積と一次モーメントをセグメント木で区間集約**: [`abc472_f_SegTree.cpp`](archive/26_08_22/abc472_f_SegTree.cpp)
- **累積和の最小値管理・セグ木で(合計値,最小累積値)をmerge・A(+1)B(-1)の区間累積和が常に非負かを判定**: [`abc473_f_SegTree.cpp`](archive/26_08_29/abc473_f_SegTree.cpp)
- **DP・複数区間から合計・セグ木で区間和クエリ・modint**: [`abc179_d_DP_mint_SegTree.cpp`](archive/26_09_01/abc179_d_DP_mint_SegTree.cpp)
- **状態パターン・座標圧縮・クエリで状態反転・セグ木で後ろで状態の個数を取得**: [`abc475_e_SegTree.cpp`](archive/26_09_12/abc475_e_SegTree.cpp)
...
- **3文字(A/B/C)の部分文字列の計数・プレフィックス差分(基準からの増減)・2文字一致・3文字一致**:[`abc455_e_combi.cpp`](archive/26_04_25/abc455_e_combi.cpp)
...
- **2次元累積和・長方形領域・片方の辺を固定して、もう片方の辺に対して満たす場合をカウント**: [`abc461_d_2D_sum.cpp`](archive/26_06_06/abc461_d_2D_sum.cpp)

#### **高度な累積和**
**キーワード**: `MEX`, `高度`, `上級累積和`, `mod`, `円環`, `多次元累積和`
...
- **6次元累積和・包除原理・ビット全探索**: [`abc465_f_6Dsum.cpp`](archive/26_07_04/abc465_f_6Dsum.cpp)
...
- **2次元累積和・k×k部分行列の中央値最小化・二分探索で中央値判定**: [`abc203_d_BinSearch_sum.cpp`](archive/26_07_10/abc203_d_BinSearch_sum.cpp)
- **部分列の勝率最大化・累積和+判定二分探索（条件付き最小値管理）**: [`abc469_e_BinSearch.cpp`](archive/26_08_02/abc469_e_BinSearch.cpp)
...
#### **キュー（Queue）・ベクター・Deque**
**キーワード**: `キュー`, `Queue`, `ベクター`, `vector`, `配列操作`, `deque`, `両端キュー`
...
- **deque・k進数変換・進数間比較**: [`abc465_d_deque.cpp`](archive/26_07_04/abc465_d_deque.cpp)
...
- **包除原理・ビット全探索・共通文字集合のをカウントして累乗計算**: [`abc246_f_inclusion-exclusion_mint.cpp`](archive/26_07_02/abc246_f_inclusion-exclusion_mint.cpp)
- **modint・組み合わせ・平方和と和の二乗の変形で答えを計算**: [`abc471_e_mint_combi.cpp`](archive/26_08_15/abc471_e_mint_combi.cpp)
...
- **平方数・約数簡約＋二分探索**: [`abc254_d_Square.cpp`](archive/26_04_21/abc254_d_Square.cpp)
- **素因数除去・与えられた数の素因数を集め、その素因数を持たない数を列挙・(すべての数に対して最大公約数が1となる数を求める・最小公倍数)**: [`abc215_d_prime.cpp`](archive/26_06_24/abc215_d_prime.cpp)
...
- **双六・確率DP・modint**: [`abc298_e_DP_mint.cpp`](archive/25_10_10/abc298_e_DP_mint.cpp)
- **桁数・modint**: [`abc460_e_mod_mint.cpp`](archive/26_05_30/abc460_e_mod_mint.cpp)
...
- **XOR・ビットごとの偶奇反転・差分管理**: [`abc470_c_xor.cpp`](archive/26_08_08/abc470_c_xor.cpp)
...
- **セグ木(ACL)での転倒数**: [`arc225_d_inversion_Segtree.cpp`](archive/26_07_19/arc225_d_inversion_Segtree.cpp)
...
- **転倒数から配列を復元・固定した25^2との差を商と余りで調整**: [`arc229_a_inversion.cpp`](archive/26_09_05/arc229_a_inversion.cpp)
...
- **優先度付きトポロジカルソート（辞書順最小）**: [`abc223_d_priority_queue_topological.cpp`](archive/26_06_14/abc223_d_priority_queue_topological.cpp)
...
- **Zアルゴリズム・接頭辞との一致数（LCP・回転文字列照合）・文字列シフト**: [`abc430_e_LCP_z_algorithm.cpp`](archive/25_11_01/abc430_e_LCP_z_algorithm.cpp)
...
- **文字列を隣接文字を異なるように再配置・偶数奇数の順に貪欲に埋める**: [`abc459_d_border.cpp`](archive/26_05_23/abc459_d_border.cpp)

#### **ローリングハッシュ**
**キーワード**: `rolling hash`, `ローリングハッシュ`, `ハッシュ`, `部分文字列`, `一致判定`
- **多重素数ローリングハッシュ・部分文字列一致判定**: [`tessoku_book_bd_rolling-hash.cpp`](archive/26_07_03/tessoku_book_bd_rolling-hash.cpp)
- **セグ木で区間ハッシュ管理・文字更新・回文判定**: [`abc331_f_rolling-hash_Segtree.cpp`](archive/26_07_03/abc331_f_rolling-hash_Segtree.cpp)
...
- **隣接要素の二倍差分で最小操作回数を判定**: [`arc229_b_adjacent_diff.cpp`](archive/26_09_05/arc229_b_adjacent_diff.cpp)
- **両端に最大を配置し、偶奇を交互に並べて最小化**: [`arc229_c_even-odd.cpp`](archive/26_09_05/arc229_c_even-odd.cpp)
...
#### **倍加・ダブリング**
**キーワード**: `ダブリング`, `倍加`, `指数的遷移`, `高速化`, `繰り返し二乗法`
...
- **各剰余の状態からの、2^k回の操作それぞれの累積和をダブリングで計算**: [`abc241_e_double.cpp`](archive/26_05_14/abc241_e_double.cpp)
